...
def main():
    base = len(antigravity_records())
    repl = Repl(HOME, HOME / "cache" / "logs" / "agy-resume-pty.log")
    try:
        time.sleep(3)
        ...
        repl.close()
    records = antigravity_records()[base:]
    assert len(records) >= 2, f"应有两条 antigravity 请求记录,实得 {len(records)}"
    first, second = records[0], records[-1]
    a1, a2 = first["body"]["args"], second["body"]["args"]
    print("turn1 resume:", "--conversation" in a1, "| turn2 resume:", "--conversation" in a2)
    print("turn2 stdin:", second["body"]["stdin"][:200])
    assert "--conversation" not in a1, "首轮不该续传"
    assert "--conversation" in a2, "第二轮应命中前缀链走 --conversation"
