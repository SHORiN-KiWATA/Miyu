...
状态：**设计稿，待拍板**。基于 09-04 对 Miyu 各域真实实现的复核（不是 AstrBot 原型），原型只当功能启发。上一份分期方案见 `docs/plan/2026-09-03-plugin-dashboard-and-io.md`，本文覆盖其"一、插件 Dashboard 移植"部分。
...
rail 顺序：用量 · 记忆 · 知识库 · 表情包 · 群聊 · 群管 · 好感·情绪 · 设置
...
| P6 | 情绪功能（另一份文档）→ 情绪标签 | 桩 LLM 五回合 + 判官阈值差取证 |
| P7 | 裁决结构化写入 → 裁决标签；定时消息标签 | 与文本日志逐条对账 |
