...
- 新成员注册后有一个三步引导：先决定「她是谁」（直接用 Miyu，或创建只属于自己的人格：起名、写设定或选模板、传头像和看板图），再勾她能用的功能（记忆、管理员放行的插件），最后写「希望她怎么认识你」。自建人格的记忆、技能、脚本都在成员自己的目录里；随时能在账号页里切换、编辑、删除或再建。管理员在设置页「成员账号」里决定成员能不能自建人格、能勾哪些插件。
